...
size_t RightEdgeCoordinateCalculator::GetCoordinate(const cube::Cube& cube) const {
    ...
    for (int i = 0; i < edges_cnt_; ++i) {
        const auto& cubie = edge_cubies[i + (cube::EDGE_INDEX_CNT - edges_cnt_)];
        rotation_coord *= 2;
...
cube::Cube RightEdgeCoordinateCalculator::GetCoordinateCube(const size_t coord) const {
    ...
    for (size_t i = 0; i < edges_cnt_; ++i) {
        ...
        edge_cubies[i + (cube::EDGE_INDEX_CNT - edges_cnt_)] = cube::GetEdgeCubie(elems[elem_index], 0);
        elems.erase(elems.begin() + elem_index);
        position_coord %= step;
    }
    size_t rotation_coord = coord % utils::POWERS_OF_TWO[edges_cnt_];
    for (int i = edges_cnt_ - 1; i >= 0; --i) {
      edge_cubies[i + (cube::EDGE_INDEX_CNT - edges_cnt_)] |= (rotation_coord % 2);
        rotation_coord /= 2;
    }
    return cube::Cube(edge_cubies, std::array<uint8_t, cube::CORNER_INDEX_CNT>{});
}

size_t RightEdgeCoordinateCalculator::Size() const {
