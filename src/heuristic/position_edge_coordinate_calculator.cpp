...
size_t PositionEdgeCoordinateCalculator::GetCoordinate(const cube::Cube& cube) const {
    ...
    for (int i = 0; i < cube::EDGE_INDEX_CNT; ++i) {
        const auto& cubie = edge_cubies[i];
...
cube::Cube PositionEdgeCoordinateCalculator::GetCoordinateCube(const size_t coord) const {
    ...
    for (size_t i = 0; i < cube::EDGE_INDEX_CNT; ++i) {
        const auto step = utils::FACTORIAL[cube::EDGE_INDEX_CNT - i - 1];
...
size_t PositionEdgeCoordinateCalculator::Size() const {
    return utils::FACTORIAL[cube::EDGE_INDEX_CNT] * utils::POWERS_OF_TWO[edges_cnt_];
}
